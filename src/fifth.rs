...
impl<'a, T> Iterator for IterMut<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        unsafe {
            self.nexti.take().map(|node| {
                self.nexti = node.next.as_mut();
                &mut node.elem
            })
        }
    }
